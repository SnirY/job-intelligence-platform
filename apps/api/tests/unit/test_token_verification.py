...
@pytest.fixture
def verifier(factory: TokenFactory) -> Iterator[ClerkTokenVerifier]:
    with serve_jwks(factory) as jwks_url:
        yield ClerkTokenVerifier(
            jwks_url,
            issuer=ISSUER,
...
def test_valid_token_yields_its_identity(
    verifier: ClerkTokenVerifier, factory: TokenFactory
) -> None:
...
def test_expired_token_is_rejected(verifier: ClerkTokenVerifier, factory: TokenFactory) -> None:
...
def test_token_signed_by_a_different_key_is_rejected(verifier: ClerkTokenVerifier) -> None:
...
def test_unknown_key_id_is_rejected(verifier: ClerkTokenVerifier) -> None:
...
def test_unsigned_token_is_rejected(verifier: ClerkTokenVerifier) -> None:
...
def test_unauthorized_party_is_rejected(
    verifier: ClerkTokenVerifier, factory: TokenFactory
) -> None:
    with pytest.raises(TokenVerificationError):
...
def test_malformed_tokens_are_rejected(verifier: ClerkTokenVerifier, malformed: str) -> None:
    with pytest.raises(TokenVerificationError):
