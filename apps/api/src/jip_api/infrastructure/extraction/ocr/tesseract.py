    ...
    def read(self, pages: Iterable[bytes], *, languages: str) -> OcrResult:
        ...
        for png in pages:
            with Image.open(BytesIO(png)) as image:
                # `image_to_data`, not `image_to_string`. The string form throws
                # the confidence away, and confidence is the whole reason a
                # reviewer can be told which parts were guessed at.
